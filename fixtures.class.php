...
class fixtures
{
    protected function getFixtures()
    {
        ...
        if ($res['res']) {
            ...
            foreach ($res['res'] as $vals) {
                ...
                $res['res'][$x]['stddevgoals'] = $this->formatStdDev($hts['goalsArray'], $ats['goalsArray']);
    ...
    private function formatStdDev($hg, $ag)
    {
        $combo = array_merge($hg, $ag);

        $no_element = count($combo);
        $var = 0.0;
        $avg = array_sum($combo) / $no_element;
